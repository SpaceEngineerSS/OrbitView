                ...
                for (let i = 0; i < satrecsRef.current.length; i++) {
                    ...
                    if (isVisible && !isNaN(x)) {
                        ...
                        if (sat.id === currentSelectedId) {
                            point.pixelSize = 14; // Larger selection
                            ...
                            // console.log("[TLELayer] Updating selected pos for", sat.id); // DEBUG
                            onUpdateSelectedPos(Cesium.Cartesian3.clone(scratchCartesian), velocity);
                        } else {
                            point.pixelSize = 8; // Increased to 8 as requested
                            point.color = POINT_COLOR_NORMAL;
                            point.scaleByDistance = SCALE_BY_DISTANCE_NORMAL;
                            point.translucencyByDistance = TRANSLUCENCY_BY_DISTANCE;
                        }
                    } else {
                        point.show = false;
                    }
